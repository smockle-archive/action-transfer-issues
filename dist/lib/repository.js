...
export class Repository {
    ...
    transferIssue = async (issue) => {
        const { sourceOwner, sourceRepo } = issue.repository_url.match(/^(?<_>.+)\/(?<sourceOwner>.+)\/(?<sourceRepo>.+)$/).groups;
        ...
        if (duplicateIssue) {
            console.log(`Skipping issue transfer. Issue source (${source.owner}/${source.repo}#${issue.number}) already exists at destination (${this.owner}/${this.repo}#${duplicateIssue.number}).`);
            return;
        }
        ...
        const labels = issue.labels.reduce((labels, label) => {
        ...
        if (canTransferBetweenOrgs && canTransferBetweenVisibilities) {
            ...
            const sourceIssueId = (await this.#client.graphql(`query {
        repository(owner: "${source.owner}", name: "${source.repo}") {
          issue(number: ${issue.number}) {
            ...
            const destinationRepoId = (await this.#client.graphql(`query {
        repository(owner: "${this.owner}", name: "${this.repo}") {
          id
        }
      }`))?.repository.id;
            const transferredIssue = (await this.#client.graphql(`mutation {
        transferIssue(
          input: {
